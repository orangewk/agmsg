...
# PID-space-aware liveness for shell-owned lifetime and lock-owner pids below.
...
if [ -z "$LIFETIME_PID" ] || ! _agmsg_msys_pid_alive "$LIFETIME_PID"; then
...
if [ -z "$ROLE_PAIR" ]; then
  ...
  while agmsg_runtime_lock_verify "$DISPATCHER_LOCK_RESOURCE" "$$" \
    && _agmsg_msys_pid_alive "$LIFETIME_PID"; do
    refresh_identity_cache
...
ids=""
startup_attempts=0
while _agmsg_msys_pid_alive "$PARENT_PID" && [ "$startup_attempts" -lt 20 ]; do
...
if [ -z "$ids" ]; then
  _agmsg_msys_pid_alive "$PARENT_PID" || exit 0
  sleep 0.3
...
while _agmsg_msys_pid_alive "$PARENT_PID"; do
